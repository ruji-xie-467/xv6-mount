...
void pid_ns_get(struct pid_namespace* pid_ns){
    //get lock first to reduce race condition
    acquire(&pid_ns_table.lock);
...
struct pid_namespace* pid_ns_alloc() {
    //get lock first to reduce race condition
    acquire(&pid_ns_table.lock);
    for (int i = 0; i < NNAMESPACE; i++) {
        //iterate until we found an empty entry in pid_ns_table
        struct pid_namespace* pid_ns = &pid_ns_table.pid_namespaces[i];
    ...
    release(&pid_ns_table.lock);
    panic("all pid_namespace objects are occupied");
}

void pid_ns_init_ns(struct pid_namespace* pid_ns, struct pid_namespace* parent_ns) {
...
struct pid_namespace* pid_ns_new(struct pid_namespace* parent_namespace) {
    ...
    struct pid_namespace * pid_ns = pid_ns_alloc();
    //init namespace
...
int pid_ns_is_max_depth(struct pid_namespace* pid_ns){
    ...
    return depth >= PID_NAMESPACE_MAX_DEPTH;
}